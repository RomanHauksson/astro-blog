...
I wanted to recreate [MF DOOM](https://en.wikipedia.org/wiki/MF_Doom)\'s prose because it\'s famously dense with rhyme, wordplay, and pop culture references. ChatGPT can already generate lyrics as sophisicated as [Lil Pump](https://en.wikipedia.org/wiki/Lil_Pump)'s using [zero-shot prompting](https://www.promptingguide.ai/techniques/zeroshot), but is it possible to use more advanced methods to coax a language model into generating lyrics of MF DOOM\'s caliber?
